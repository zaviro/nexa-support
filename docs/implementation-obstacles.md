...
| Vercel 部署 | 首次真实预览构建不在 Git 工作树中，`prepare` 无条件运行 `lefthook install` 而失败；修复后 Vercel 日志又显示 Next 默认匿名遥测提示，并检测到被 Git 忽略的本地 `.env`。 | 首次预览为 `ERROR`，默认遥测不符合项目无追踪边界，且本地环境文件不能随直接 CLI 部署进入构建。 | 仅在 Git 工作树安装 Lefthook，在项目 `build` 脚本中显式禁用 Next 遥测，并用 `.vercelignore` 排除所有 `.env*`；不添加 Vercel 环境变量、token 或运行时服务。 | 首次 deployment `dpl_ciopDQM1TvysDvkJgr5vwYpGZKRS`；commits `28600d8`、`9cec5ff` 与后续 `.vercelignore` 修复 |
